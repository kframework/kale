...
package object kale {
  ...
  implicit class RichTerm(private val term: Term) extends AnyVal {
    ...
    def toConstructor: String =
      term match {
        ...
        case term.label.env.Variable(name) => name._1.toString
  ...
  case class foldLeft[D](d: D)(f: (D, Term) => D) extends (Term => D) {
    override def apply(t: Term): D = {
      t.flattenedChildren.foldLeft(d)(f)
    }
  }
  ...
  implicit class PFwithUnapply[A, B](pf: PartialFunction[A, B]) {
    val unapply = new Unapply[A, B] {
